...
- After a failed focused run, fix the failure and rerun only the affected package, test target, test class, or test case. For `.github/workflows/torah-ci.yml`, use `package_test_filter` for a single Swift test and set `run_ui_tests=true` only when the changed behavior requires simulator coverage.
