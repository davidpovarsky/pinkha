...
- **CI GitHub Actions** :
  ...
  - ⚠️ Swift : ne pas déclencher, attendre ni utiliser le job **`Swift tests (self-hosted, Xcode 27)`** ; ce runner est durablement indisponible/bloqué. Utiliser le workflow GitHub-hosted `.github/workflows/torah-ci.yml` et, par défaut, uniquement les tests ciblés correspondant aux changements. La matrice complète unit/integration/XCUITest (`full_suite=true`) ne doit être lancée que sur demande explicite actuelle de l'utilisateur. Les règles faisant autorité sont dans **`AGENTS.md`**.
