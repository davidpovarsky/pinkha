...
public struct LeafDecorView: View {
    ...
    let onTorahAdd: ((TorahAssociationKind) -> Void)?
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if !verrouille {
                HStack(spacing: 10) {
                    ...
                    iconMenu
                    if let onTorahAdd {
                        TorahAddLinkControl(onSelect: onTorahAdd)
                    }
                }
                .font(.caption)
    ...
    private var coverMenuContenu: some View {
        ...
        Button {
            ...
            Label("Choose a file", systemImage: "shelf")
        }
        Divider()
    ...
    private func coverImage(_ id: String) -> UIImage? {
        if !id.hasPrefix("file://") && !id.hasPrefix("cover.") {
            guard let directory = try? LeafViewModel.coversDirectory() else { return nil }
    ...
    private func loadCoverCache(id: String) async {
        ...
        if id.hasPrefix("file://") {
            ...
            path = nil
        } else if let directory = try? LeafViewModel.coversDirectory() {
