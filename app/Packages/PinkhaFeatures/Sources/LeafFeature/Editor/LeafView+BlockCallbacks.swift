...
public extension LeafView {
    ...
    func blockCallbacks(for block: EditableBlock) -> BlockCallbacks {
        BlockCallbacks(
            ...
            // leaf-level direction (already loaded into the VM).
            ...
            // `@`-mention candidates : every leaf the user can
            // link to. We include the doc currently being edited so
            // a library with a single doc still gets a non-empty
            // list (otherwise the bar wouldn't appear on first use).
            onMentionLookup: { [weak vm] in
                guard let vm,
                      let docs = try? vm.api.listLeaves() else { return [] }
